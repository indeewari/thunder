...
type CheckerTestSuite struct {
	suite.Suite
	mockStore *RevokedTokenReaderInterfaceMock
	checker   *checker
}
...
func (s *CheckerTestSuite) SetupTest() {
	s.mockStore = NewRevokedTokenReaderInterfaceMock(s.T())
	s.checker = &checker{
		store:            s.mockStore,
...
func (s *CheckerTestSuite) TestEnsureNotRevoked_EmptyJTI() {
	err := s.checker.EnsureNotRevoked(context.Background(), "")
...
func (s *CheckerTestSuite) TestEnsureNotRevoked_OpenCircuitShortCircuits() {
	...
	for i := 0; i < enforcementFailureThreshold; i++ {
		err := s.checker.EnsureNotRevoked(context.Background(), "jti-loop")
		s.Assert().ErrorIs(err, ErrEnforcementUnavailable)
	}
...
func (s *CheckerTestSuite) TestEnsureNotRevoked_AlertsOncePerTrip() {
...
// is published.
func (s *CheckerTestSuite) TestEnsureNotRevoked_DisabledObservabilityDoesNotPublish() {
	obsMock := observabilitymock.NewObservabilityServiceInterfaceMock(s.T())
...
func (s *CheckerTestSuite) TestEnsureNotRevoked_RecoversAfterCooldown() {
	s.mockStore.On("IsTokenRevoked", mock.Anything, "jti-recover").
		Return(false, errors.New("db down")).Times(enforcementFailureThreshold)
	for i := 0; i < enforcementFailureThreshold; i++ {
		_ = s.checker.EnsureNotRevoked(context.Background(), "jti-recover")
	}

	// Simulate the cooldown elapsing, then let the store recover.
	s.checker.breaker.openedAt = time.Now().Add(-2 * enforcementOpenDuration)
	s.mockStore.On("IsTokenRevoked", mock.Anything, "jti-recover").Return(false, nil)

	err := s.checker.EnsureNotRevoked(context.Background(), "jti-recover")
	s.Assert().NoError(err)
	s.Assert().True(s.checker.breaker.allow(), "circuit should be closed after a successful trial call")
}
